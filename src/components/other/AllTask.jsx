import React from "react";

const AllTask = () => {
  return (
    <div className="bg-[#1c1c1c] p-5 mt-5 rounded h-48 overflow-auto">
      <div className="bg-red-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-green-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-blue-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-yellow-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-orange-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-purple-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-pink-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-emerald-200 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
      <div className="bg-red-400 py-2 px-4 flex justify-between rounded mb-2">
        <h2>Nandini</h2>
        <h3>Plan a Meeting</h3>
        <h5>Status</h5>
      </div>
    </div>
  );
};

export default AllTask;
